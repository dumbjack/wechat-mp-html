...
Omit any field that is not present in the original article. Supported aliases: `data_cards` for `data`, `judgement_cards` or `highlight_cards` for `highlights`, and `closing.question` for `closing_question`.
